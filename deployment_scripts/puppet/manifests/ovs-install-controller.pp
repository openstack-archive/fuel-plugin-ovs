...
if $operatingsystem == 'Ubuntu' {
        ...
        if $fuel_settings['fuel-plugin-ovs']['use_dppd'] {
                exec { "wget dppd package":
                        command => "wget http://10.20.0.2:8080/plugins/fuel-plugin-ovs-0.5/repositories/ubuntu/dppd-install.tar.gz",
                        path   => "/usr/bin:/usr/sbin:/bin:/sbin",
                }
                exec { "unzip dppd package":
                        command => "tar -xvzf /etc/fuel/plugins/fuel-plugin-ovs-0.5/dppd-install.tar.gz",
                        path   => "/usr/bin:/usr/sbin:/bin:/sbin",
                }
                exec { "install dppd package":
                        command => "/etc/fuel/plugins/fuel-plugin-ovs-0.5/dppd-install/dppd-install.sh",
                        path   => "/usr/bin:/usr/sbin:/bin:/sbin",
                }
        }
